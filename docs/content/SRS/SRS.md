...
This following model represents the core entities and relationships within tinymorph, focusing on **Users**, **Documents**, **Preferences**, **Models**, and **Feedback**. These entities form the backbone of tinymorph's functionality, allowing users to create, edit, and manage text while interacting with AI-driven models for enhanced text generation.

**Entities and Relationships**

| **Entity**      | **Attributes**                                                   | **Relationships**                                                                 |  
|-----------------|------------------------------------------------------------------|----------------------------------------------------------------------------------|
| **User**        | UserID, Username, Email, PreferencesID, LastLogin                | Can create, edit, and manage **Documents** and **Preferences**; provides **Feedback** on text suggestions |
| **Document**    | DocumentID, Title, Content, Sections, Status, Revisions, CreatedDate, LastModifiedDate | Created and owned by **User**; contains multiple **Sections** and **Revisions**   |
| **Preferences** | PreferencesID, Tone, Style, Voice, Formality, Language, SavedConfigurations   | Linked to **User**; influences the behavior of the **Model** during text generation |
| **Model**       | ModelID, Name (e.g., GPT, Llama 3), Version, Status, LastRunTime  | Generates text for **Documents** based on **Preferences**; influenced by **Feedback**  |
| **Feedback**    | FeedbackID, TextSnippet, Action (accept/reject), Timestamp        | Linked to **User**, applied to **Document** and **Model** behavior                |


### 7.2 Data Dictionary

The following data dictionary is a formal specification of the entities, attributes, and relationships within tinymorph. It includes:
- Classes in the data model
- Attributes of each class
- Relationships between classes
- Inputs and outputs for data flow
- Definitions of the different data elements

**Data Dictionary for tinymorph**

| **Term**                  | **Type**            | **Definition**                                                                                  | **Input/Output**                        |
|---------------------------|---------------------|-------------------------------------------------------------------------------------------------|-----------------------------------------|
| **UserID**                 | Attribute           | A unique identifier assigned to each user in the system.                                         | Input (user-provided on registration)   |
| **Username**               | Attribute           | The display name chosen by the user to represent their identity in the system.                   | Input (user-provided)                   |
| **Email**                  | Attribute           | The user’s email address used for communication and login.                                       | Input (user-provided)                   |
| **PreferencesID**          | Attribute           | A unique identifier linking a user to their preferences and settings.                            | Input (generated by system)             |
| **LastLogin**              | Attribute           | The timestamp marking the last time the user accessed the system.                                | Output (recorded by system)             |
| **DocumentID**             | Attribute           | A unique identifier for each document created in tinymorph.                                      | Input (generated by system)             |
| **Title**                  | Attribute           | The title of the document created by the user.                                                   | Input (user-provided)                   |
| **Content**                | Attribute           | The main body text of the document, which could be user-inputted or AI-generated.                | Input/Output (user-provided or generated) |
| **Sections**               | Class               | Represents individual parts of a document (e.g., Introduction, Conclusion).                      | Output (generated by system)            |
| **Status**                 | Attribute           | The current state of the document (e.g., Draft, Completed, In Review).                           | Output (generated by system)            |
| **Revisions**              | Class               | Tracks the history of changes made to the document, including the timestamp and content edits.    | Output (generated by system)            |
| **CreatedDate**            | Attribute           | The date when the document was initially created.                                                | Output (generated by system)            |
| **LastModifiedDate**       | Attribute           | The date of the most recent modification made to the document.                                   | Output (generated by system)            |
| **Tone**                   | Attribute           | The emotional tone (e.g., formal, casual) the user wants the document to convey.                  | Input (user-provided)                   |
| **Style**                  | Attribute           | The user’s preferred style of writing (e.g., descriptive, persuasive).                           | Input (user-provided)                   |
| **Voice**                  | Attribute           | The narrative voice (e.g., first-person, third-person) selected by the user.                     | Input (user-provided)                   |
| **Formality**              | Attribute           | The level of formality (e.g., conversational, professional) the user desires for their text.      | Input (user-provided)                   |
| **SavedConfigurations**    | Class               | A set of user-saved configurations for different writing contexts (e.g., academic, creative).     | Input/Output (saved by user/system)     |
| **ModelID**                | Attribute           | A unique identifier for the language model (e.g., GPT, Llama 3) used for generating text.         | Output (generated by system)            |
| **Name**                   | Attribute           | The name of the language model (e.g., GPT, Llama 3) used in tinymorph.                           | Output (generated by system)            |
| **Version**                | Attribute           | The version number of the language model being used.                                             | Output (generated by system)            |
| **Status (Model)**         | Attribute           | The current operational state of the model (e.g., Active, Updating, Inactive).                   | Output (generated by system)            |
| **LastRunTime**            | Attribute           | The timestamp indicating the last time the model was invoked for text generation.                | Output (generated by system)            |
| **FeedbackID**             | Attribute           | A unique identifier for each instance of feedback provided by the user.                          | Output (generated by system)            |
| **TextSnippet**            | Element             | A specific portion of text, either generated by the model or user, which the feedback refers to.  | Input/Output (generated by system/user) |
| **Action**                 | Attribute           | The action taken by the user in response to the model’s suggestion (e.g., accept, reject).        | Input (user-provided)                   |
| **Timestamp**              | Attribute           | The date and time the feedback was provided, used for tracking and historical reference.          | Output (generated by system)            |
...
The dictionary ensures that all data elements are consistent with the system’s business logic and aids in implementation by providing exact definitions for each term. During implementation, the dictionary will evolve to include more technical details, such as data formats, integration points, and validation rules.
...
### Additional Functional Requiremnts ###

FR-15. Users shall be able to click on a specific part of the text and select from a dropdown of alternative phrasings or sentence structures generated by the model.
...
FR-16. The system shall support an offline locally hosted language models for text generation through on-device inference.
