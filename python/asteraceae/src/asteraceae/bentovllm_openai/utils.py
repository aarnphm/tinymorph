...
def openai_endpoints(
  ...
  def openai_wrapper(svc: Service[T]):
    ...
    if default_completion_parameters is not None:
      ...
      assert CompletionRequest(prompt='', model='', **default_completion_parameters)

    if default_chat_completion_parameters is not None:
      assert 'messages' not in default_chat_completion_parameters
      assert ChatCompletionRequest(messages=[], model='', **default_chat_completion_parameters)
...
def _make_httpx_client(url, svc):
  ...
  return httpx.Client(transport=transport, timeout=timeout, follow_redirects=True, headers=headers), target_url
